...
bool ed25519_verify_message(const std::string &message, const std::string &sig_b64, const std::string &pubkey_b64)
{
  ...
  {
    const char *env_pk = std::getenv("KERNEL_CONTROLLER_PUBKEY_B64");
    if (env_pk && *env_pk)
      pubkey_copy = env_pk;
    else
    {
      ...
      if (dpapi_load_blob_to_b64("KERNEL_CONTROLLER_PUBKEY_DPAPI_B64", "KERNEL_CONTROLLER_PUBKEY_DPAPI_PATH", dpapi_b64))
      {
        pubkey_copy = dpapi_b64;
      }
      else
      {
        const char *pk_path = std::getenv("KERNEL_CONTROLLER_PUBKEY_PATH");
        if (pk_path)
        {
